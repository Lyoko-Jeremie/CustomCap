...
const Archiver = {
  ...
  archive: ( archive: Archive ): Uint8Array => {
    ...
    const [n, a, t, Ck, Cm, nonce] = archive;
    ...
    const Cm_length = Int32.encode ( Cm.length );

    const nonce_length = Int32.encode ( nonce.length );

    return concat ([ n_length, n_uint8, a_length, a_uint8, t_length, t_uint8, Ck_length, Ck_uint8, Cm_length, Cm, nonce_length, nonce ]);
  ...
  unarchive: ( archive: Uint8Array ): Archive => {
    ...
    const Cm_length_offset = Ck_offset + Ck_length;
    const Cm_length_uint8 = archive.slice ( Cm_length_offset, Cm_length_offset + 4 );
    const Cm_length = Int32.decode ( Cm_length_uint8 );
    const Cm_offset = Cm_length_offset + 4;
    const Cm = archive.slice ( Cm_offset, Cm_offset + Cm_length );

    const nonce_length_offset = Cm_offset + Cm_length;
    const nonce_length_uint8 = archive.slice ( nonce_length_offset, nonce_length_offset + 4 );
    const nonce_length = Int32.decode ( nonce_length_uint8 );
    const nonce_offset = nonce_length_offset + 4;
    const nonce = archive.slice ( nonce_offset, nonce_offset + nonce_length );
